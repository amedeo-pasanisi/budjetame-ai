...
const SECTION_LABELS: Record<CategoryType, string> = {
  expense: 'categories.section.expenses',
  income: 'categories.section.incomes',
}
...
export function CategoriesScreen({
  ...
  useEffect(() => {
    ...
    fetchCategories(token)
      ...
      .catch(() => {
        if (!cancelled) setLoadError(formatMessage({ id: 'categories.loadError' }))
      })
    return () => {
      cancelled = true
    }
  ...
  const sections = useMemo(() => {
    ...
    return SECTION_TYPES.map((type) => ({
      type,
      label: formatMessage({ id: SECTION_LABELS[type] }),
      items: filtered
        .filter((category) => category.type === type)
        .sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })),
    }))
  }, [categories, query, formatMessage])

  return (
    <>
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-slate-900">{formatMessage({ id: 'categories.title' })}</h2>
        <button
          type="button"
      ...
      {categories === null ? (
        <p className="mt-3 text-sm text-slate-500">{formatMessage({ id: 'categories.loading' })}</p>
      ) : categories.length === 0 ? (
        ...
        <>
          <div className="mt-3">
            <input
              aria-label={formatMessage({ id: 'categories.searchLabel' })}
              type="search"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder={formatMessage({ id: 'categories.searchPlaceholder' })}
          ...
          {sections !== null && sections.every((section) => section.items.length === 0) ? (
            <p className="mt-3 text-sm text-slate-500">{formatMessage({ id: 'categories.noSearch' })}</p>
          ) : (
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1">
                              ...
                              <span className="block text-xs text-slate-500">
                                {formatMessage({ id: category.type === 'expense' ? 'categories.type.expense' : 'categories.type.income' })}
                              </span>
                            </span>
                          </button>
                          <button
                            type="button"
                            aria-label={formatMessage({ id: 'categories.editLabel' }, { name: category.name })}
