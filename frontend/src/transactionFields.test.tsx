...
describe('TypeSelector', () => {
  it('renders the three transaction type buttons', () => {
    renderWithIntl(<TypeSelector active="expense" disabled={false} onSelect={() => {}} />)
  ...
  it('reports a click with the selected type', () => {
    ...
    renderWithIntl(<TypeSelector active="expense" disabled={false} onSelect={onSelect} />)
    fireEvent.click(screen.getByRole('button', { name: 'Transfer' }))
    expect(onSelect).toHaveBeenCalledWith('transfer')
  })

  it('disables the type buttons while a transaction is being saved', () => {
    renderWithIntl(<TypeSelector active="expense" disabled={true} onSelect={() => {}} />)
