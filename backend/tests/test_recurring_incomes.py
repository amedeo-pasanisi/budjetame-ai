...
an optional start date (defaults to the creation date when unset), and an
optional due-date override (day-of-month for months, month+day for years,
none for day/week). The Wallet and Category of a linked Income are chosen at
Transaction creation time — the definition itself never carries them.
...
unique per Account case-insensitively; all data scoped to the Account
(foreign data is a 403, ADR-0003).
...
async def test_create_recurring_income(client: AsyncClient) -> None:
    ...
    response = await client.post(
        "/recurring-incomes",
        json=_income(start_date="2030-03-15", due_day=1),
        headers=_auth(token),
    )
...
async def test_override_rules_follow_the_interval_unit(client: AsyncClient) -> None:
    ...
    for overrides in cases:
        response = await client.post(
            "/recurring-incomes",
            json=_income(name=f"Override {overrides}", **overrides),
            headers=_auth(token),
        )
...
async def test_edit_recurring_income(client: AsyncClient) -> None:
    ...
    created = await client.post(
        "/recurring-incomes",
        json=_income(name="Edit Salary", start_date="2030-03-15", due_day=1),
        headers=_auth(token),
    )
...
async def test_edit_can_clear_the_start_date_and_the_override(
    ...
    created = await client.post(
        "/recurring-incomes",
        json=_income(name="Clearable Salary", start_date="2030-03-15", due_day=1),
        headers=_auth(token),
    )
    income_id = created.json()["id"]
...
async def test_edit_rejects_an_override_stale_after_a_unit_change(
    ...
    created = await client.post(
        "/recurring-incomes",
        json=_income(name="Stale Salary", start_date="2030-03-15", due_day=1),
        headers=_auth(token),
    )
    income_id = created.json()["id"]
