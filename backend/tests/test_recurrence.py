...
def test_next_due_for_days_walks_to_the_first_due_not_before_today() -> None:
    ...
    assert next_due_date(start, 5, "days", None, None, date(2026, 1, 6), set()) == date(2026, 1, 6)
    assert next_due_date(start, 5, "days", None, None, date(2026, 1, 7), set()) == date(2026, 1, 11)
    assert next_due_date(start, 5, "days", None, None, date(2026, 2, 11), set()) == date(2026, 2, 15)


def test_next_due_with_a_monthly_override() -> None:
    # Sequence from the 15th, due on the 1st: the first day of each month.
    start = date(2026, 3, 15)
    assert next_due_date(start, 1, "months", 1, None, date(2026, 2, 10), set()) == date(2026, 3, 1)
    assert next_due_date(start, 1, "months", 1, None, date(2026, 3, 1), set()) == date(2026, 3, 1)
    assert next_due_date(start, 1, "months", 1, None, date(2026, 3, 2), set()) == date(2026, 4, 1)


def test_next_due_walks_across_clamped_months() -> None:
    # Start on the 31st, due on the 31st: dues are 01-31, 02-28, 03-31, ...
    start = date(2026, 1, 31)
    assert next_due_date(start, 1, "months", 31, None, date(2026, 2, 10), set()) == date(2026, 2, 28)
    assert next_due_date(start, 1, "months", 31, None, date(2026, 3, 1), set()) == date(2026, 3, 31)


def test_next_due_for_a_yearly_override() -> None:
    start = date(2026, 5, 10)
    assert (
        next_due_date(start, 1, "years", 1, 12, date(2026, 11, 30), set()) == date(2026, 12, 1)
    )
    assert (
        next_due_date(start, 1, "years", 1, 12, date(2027, 1, 1), set()) == date(2027, 12, 1)
    )
...
def test_next_due_with_an_early_override_skips_a_past_first_due() -> None:
    ...
    assert next_due_date(start, 1, "months", 1, None, date(2026, 12, 15), set()) == date(2027, 1, 1)


def test_next_due_before_the_start_is_the_first_occurrence() -> None:
    start = date(2026, 3, 15)
    assert next_due_date(start, 1, "months", None, None, date(2026, 2, 1), set()) == date(2026, 3, 15)


def test_next_due_rejects_unknown_unit() -> None:
    with pytest.raises(ValueError):
        next_due_date(date(2026, 1, 1), 1, "fortnights", None, None, date(2026, 1, 1), set())
...
def test_window_walker_days_interval_with_inclusive_edges() -> None:
    ...
    assert occurrences_in_window(
        start, 5, "days", None, None, date(2026, 1, 6), date(2026, 1, 31), set()
    ) == [
        date(2026, 1, 6),
        date(2026, 1, 11),
        date(2026, 1, 16),
...
def test_window_walker_weeks_interval() -> None:
    ...
    start = date(2026, 1, 1)
    assert occurrences_in_window(
        start, 2, "weeks", None, None, date(2026, 1, 15), date(2026, 2, 12), set()
...
def test_window_walker_months_with_clamping() -> None:
    ...
    start = date(2026, 1, 31)
    assert occurrences_in_window(
        start, 1, "months", 31, None, date(2026, 2, 1), date(2026, 4, 30), set()
...
def test_window_walker_months_with_an_override() -> None:
    ...
    start = date(2026, 3, 15)
    assert occurrences_in_window(
        start, 1, "months", 1, None, date(2026, 3, 1), date(2026, 5, 31), set()
...
def test_window_walker_uses_leap_february() -> None:
    ...
    start = date(2028, 1, 31)
    assert occurrences_in_window(
        start, 1, "months", None, None, date(2028, 2, 1), date(2028, 4, 30), set()
...
def test_window_walker_years_with_an_override() -> None:
    ...
    start = date(2026, 5, 10)
    assert occurrences_in_window(
        start, 1, "years", 1, 12, date(2026, 12, 1), date(2026, 12, 31), set()
    ) == [date(2026, 12, 1)]
    assert occurrences_in_window(
        start, 1, "years", 1, 12, date(2026, 1, 1), date(2026, 11, 30), set()
    ) == []


def test_window_walker_before_the_start_is_empty() -> None:
    assert occurrences_in_window(
        date(2026, 3, 15), 1, "months", None, None, date(2026, 1, 1), date(2026, 2, 28), set()
    ) == []
...
def test_window_walker_an_early_override_pulls_the_first_due_in() -> None:
    ...
    start = date(2026, 12, 31)
    assert occurrences_in_window(
        start, 1, "months", 1, None, date(2026, 12, 1), date(2026, 12, 31), set()
    ) == [date(2026, 12, 1)]
...
def test_window_walker_a_gap_between_dues_is_empty() -> None:
    ...
    assert occurrences_in_window(
        date(2026, 1, 1), 5, "days", None, None, date(2026, 1, 2), date(2026, 1, 5), set()
    ) == []
...
def test_window_walker_a_full_month_of_daily_dues() -> None:
    ...
    dues = occurrences_in_window(
        start, 1, "days", None, None, date(2026, 1, 1), date(2026, 1, 31), set()
    )
    assert len(dues) == 31
...
def test_backlog_counts_unpaid_occurrences_due_today_or_earlier() -> None:
    ...
    start = date(2026, 1, 1)
    assert backlog_count(start, 1, "days", None, None, today, set(), set()) == 10
...
def test_backlog_skips_occurrences_covered_by_a_pin() -> None:
    ...
    assert backlog_count(start, 1, "days", None, None, today, paid, set()) == 8
    ...
    assert backlog_count(start, 1, "days", None, None, today, paid_elsewhere, set()) == 10
...
def test_backlog_counts_today_but_never_future_occurrences() -> None:
    ...
    today = date(2026, 1, 10)
    assert backlog_count(date(2026, 1, 10), 1, "days", None, None, today, set(), set()) == 1
    assert backlog_count(date(2026, 1, 11), 1, "days", None, None, today, set(), set()) == 0
...
def test_backlog_with_a_monthly_override_judges_the_due_date() -> None:
    ...
    assert backlog_count(start, 1, "months", 1, None, today, set(), set()) == 4
    ...
    assert backlog_count(start, 1, "months", 1, None, today, paid, set()) == 2
...
def test_backlog_with_a_yearly_override() -> None:
    ...
    assert backlog_count(start, 1, "years", 1, 12, date(2026, 5, 1), set(), set()) == 3
    assert backlog_count(start, 1, "years", 1, 12, date(2026, 12, 1), set(), set()) == 4
...
def test_backlog_walks_across_clamped_months() -> None:
    ...
    assert backlog_count(start, 3, "months", None, None, date(2026, 1, 10), set(), set()) == 3


def test_backlog_before_the_start_is_zero() -> None:
    assert backlog_count(date(2026, 3, 15), 1, "months", None, None, date(2026, 2, 1), set(), set()) == 0


def test_backlog_rejects_unknown_unit() -> None:
    with pytest.raises(ValueError):
        backlog_count(date(2026, 1, 1), 1, "fortnights", None, None, date(2026, 1, 1), set(), set())
...
def test_next_due_walks_past_a_skipped_occurrence() -> None:
    ...
    assert (
        next_due_date(start, 5, "days", None, None, date(2026, 1, 6), skipped)
        == date(2026, 1, 11)
    )


def test_next_due_walks_past_a_skipped_month_with_an_override() -> None:
    # The rent example: Occurrences on the 15th, due on the 1st. Skipping
    # March's Occurrence pushes the next due from 03-01 to 04-01.
    start = date(2026, 3, 15)
    skipped = {(2026, 3)}
    assert (
        next_due_date(start, 1, "months", 1, None, date(2026, 2, 10), skipped)
        == date(2026, 4, 1)
    )
...
def test_backlog_never_counts_a_skipped_occurrence() -> None:
    ...
    assert backlog_count(start, 1, "days", None, None, today, set(), skipped) == 7
...
def test_backlog_mixes_paid_and_skipped() -> None:
    ...
    assert backlog_count(start, 1, "days", None, None, today, paid, skipped) == 8


def test_backlog_a_skipped_month_with_an_override_never_counts() -> None:
    # The rent example: Occurrences on the 15th, due on the 1st. Skipping
    # the October Occurrence drops the 4-deep backlog to 3.
    today = date(2026, 1, 10)
    start = date(2025, 10, 15)
    skipped = {period_of(start, "months")}  # (2025, 10)
    assert backlog_count(start, 1, "months", 1, None, today, set(), skipped) == 3
...
def test_backlog_ignores_a_dormant_skip() -> None:
    ...
    assert backlog_count(start, 1, "days", None, None, today, set(), skipped) == 10
